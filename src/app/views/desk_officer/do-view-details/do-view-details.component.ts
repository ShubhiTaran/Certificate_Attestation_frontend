...
@Component({
  selector: 'app-do-view-details',
  templateUrl: './do-view-details.component.html',
  styleUrls: ['./do-view-details.component.scss']
})
export class DOViewDetailsComponent implements OnInit {
  ...
  errorMsg = {
    ...
    doc_remark: "", //41
    year_of_passing_remark: "", //42
    nationality_remark: "", //43
  ...
  parmas() {
    ...
    if (chkStatus == "Approved" || chkStatus == "Rejected" || chkStatus == "Correction") {
      this.setHide = true
    }
    else {
      this.setHide = false
  ...
  errorMessage(data) {
    ...
    // console.log("key", keys, "classArr", this.classArr )
  }
  ...
  submit(primary_reason) {

    console.log("primary reson", this.primary_reason);

    if (!this.ApproveStatus) {
      Swal.fire('', this.lan.select_status)
      return
    }

    if (this.ApproveStatus == "Approved") {
      if (!this.primary_reason) {
        Swal.fire('', this.lan.please_fill_the_reason)
        return
      }
      this.remark()
      this.firstVerification()
      const msg = "Approved mail successfully sent to the applicant";
      Swal.fire('', msg)
      this.router.navigate(['../desk-officer-dashboard']);
    }
      ...
      // this.primary_reason = "Correction"
